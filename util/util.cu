...
void slowGenerateUniformPositiveDefiniteMatrix(float *dA, int n) {
  float *tmp = (float *)malloc(sizeof(*tmp) * n * n);
  generateUniformMatrix(tmp, n, n);
  for (auto i = 0U; i < n; i++) {
    for (auto j = 0U; j < n; j++) {
      dA[i + n * j] = 0;
      for (auto k = 0U; k < n; k++) {
        dA[i + n * j] += tmp[i + n * k] * tmp[j + n * k];
      }
    }
  }
}
...
__global__ void generateUniformPositiveDefiniteMatrix(float *dA, int n) {
  float *tmp;
  cudaMalloc(&tmp, sizeof(*tmp) * n * n);
  size_t i = threadIdx.x + blockDim.x * blockIdx.x;
  size_t j = threadIdx.y + blockDim.y * blockIdx.y;
  if (i < n && j < n) {
    dA[i + j * n] = tmp[i + j * n] * tmp[j + i * n];
  }
}
...
void sSubstract(cublasHandle_t handle, int m, int n, float *dA, int lda,
  ...
  float snegone = -1.0;
  float sone = 1.0;
  cublasSgeam(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n, &snegone, dA, lda, &sone,
              dB, ldb, dA, lda);
}
...
__global__ void sSubstractAndSquare(int m, int n, float *dA, int lda, float *dB,
  ...
  int j = threadIdx.y + blockDim.y * blockIdx.y;
  if (i < m && j < n) {
    dB[i + j * ldb] = (dA[i + j * lda] - dB[i + j * ldb]) *
                      (dA[i + j * lda] - dB[i + j * ldb]);
  }
}
...
void print_env() {
  ...
  cudaDeviceGetAttribute(&s2dratio, cudaDevAttrSingleToDoublePrecisionPerfRatio,
                         0);
  cudaDeviceGetAttribute(&mpcount, cudaDevAttrMultiProcessorCount, 0);
  cudaDeviceGetAttribute(&mpcount, cudaDevAttrMultiProcessorCount, 0);
  cudaDeviceGetAttribute(&mpcount, cudaDevAttrMultiProcessorCount, 0);
  ...
  std::cout << "NVCC Version: " << __CUDACC_VER_MAJOR__ << "."
            << __CUDACC_VER_MINOR__ << std::endl;
