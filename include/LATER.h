...
rsyrk: recursive symmetric rank k C := alpha*A*A**T + beta*C or C := alpha*A**T*A + beta*C
rtrmm: recursive B := alpha*op(A)*B or B := alpha*B*op(A) where op(A) is one of A, A**T, or A**H
*/
...
template <typename T> void printMatrixDeviceBlock(char *filename, int m, int n, T *dA, int lda) {
