...
public:
  ...
  OC_gemm(int _M, int _N, int _K, std::shared_ptr<Mem_pool> _pool,
          size_t _mem_limit = 0, int _num_stream = 2);
  ~OC_gemm();
  void gemm(cublasOperation_t transa, cublasOperation_t transb,
            const float &alpha, const half *A, int lda, const half *B, int ldb,
