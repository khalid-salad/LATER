...
int main(int argc, char *argv[]) {
  ...
  // printf("debug 1\n");
  cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n, m, &sone, A, m, B, m,
              &szero, work, m);
  ...
  if (checkFlag) {
    ...
    cublasSgeam(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n, &sone, dC, m, &snegone,
                C, m, dC, m);
...

  {
    ...
    cublasStrmm(handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N,
                CUBLAS_DIAG_NON_UNIT, m, n, &sone, A, m, B, m, C, m);
