...
int main(int argc, char *argv[]) {
    ...
    // reference implementation in cuSOLVER
    {
        generateUniformMatrix(A, m, n);
        int lwork = 0;
        auto status = cusolverDnSgeqrf_bufferSize(ctxt.cusolver_handle, m, n, A, m, &lwork);
        ...
        startTimer();
        status = cusolverDnSgeqrf(ctxt.cusolver_handle, m, n, A, m, d_tau, d_work, lwork, devInfo);
        assert(CUSOLVER_STATUS_SUCCESS == status);
        float ms = stopTimer();
...
void sgemm(int m, int n, int k, float *dA, int lda, float *dB, int ldb, float *dC, int ldc, float alpha, float beta) {
    ...
    float sone = alpha;
    float szero = beta;
    cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n, k, &sone, dA, lda, dB, ldb, &szero, dC, ldc);
...
void checkOtho(int m, int n, float *Q, int ldq) {
    ...
    cublasHandle_t handle;
    cublasCreate(&handle);
    cublasGemmEx(handle, CUBLAS_OP_T, CUBLAS_OP_N, n, n, m, &snegone, Q, CUDA_R_32F, ldq, Q, CUDA_R_32F, ldq, &sone, I,
                 CUDA_R_32F, n, CUDA_R_32F, CUBLAS_GEMM_DEFAULT);
...
void checkResult(int m, int n, float *A, int lda, float *W, int ldw, float *Y, int ldy, float *R, int ldr) {
    ...
    cublasHandle_t handle;
    cublasCreate(&handle);
    cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_T, m, n, n, &snegone, W, CUDA_R_32F, ldw, Y, CUDA_R_32F, ldy, &sone, WI,
                 CUDA_R_32F, m, CUDA_R_32F, CUBLAS_GEMM_DEFAULT);

    float normWI = snorm(m, n, WI);
    printf("normWI = %f\n", normWI);

    cublasGemmEx(handle, CUBLAS_OP_T, CUBLAS_OP_N, n, n, m, &snegone, WI, CUDA_R_32F, m, WI, CUDA_R_32F, m, &sone, I,
                 CUDA_R_32F, n, CUDA_R_32F, CUBLAS_GEMM_DEFAULT);
    ...
    cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n, n, &snegone, WI, CUDA_R_32F, m, R, CUDA_R_32F, ldr, &sone, A,
                 CUDA_R_32F, lda, CUDA_R_32F, CUBLAS_GEMM_DEFAULT);
    normRes = snorm(m, n, A);
    printf("||A-QR||/||A|| = %.6e\n", normRes / normA);
    cudaFree(I);
    cudaFree(WI);
