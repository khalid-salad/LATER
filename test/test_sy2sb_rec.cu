...
__global__ void generateSyMatrix(int m, int n, float *dA, int lda,
                                 float *tmpA) {
...
__global__ void setZeroOfWork(int m, int n, float *a, int lda) {
  int i = threadIdx.x + blockDim.x * blockIdx.x;
  int j = threadIdx.y + blockDim.y * blockIdx.y;
  if (i < m && j < n) {
    a[i + j * lda] = 0;
...
int main(int argc, char *argv[]) {
  ...
  later_rhouqr(ctxt, n, NMIN, A, lda, work, lda, work, NMIN, work, lwork, hwork,
               lhwork, work);
