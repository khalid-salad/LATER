...
__global__ void generateNewU(int m, int n, float *dA, int lda, float *tmpA) {
...
void later_qdwh_polar(cudaCtxt ctxt, int n, float *A, int lda, float *H,
  ...
  for (int iter = 0; iter < 10; iter++) {
    ...
    cublasGemmEx(ctxt.cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T, n, n, n, &alpha,
                 Q1, CUDA_R_16F, n, Q2, CUDA_R_16F, n, &beta, work, CUDA_R_32F,
...
// void recur_tri(cudaCtxt ctxt, int mm, int nn, __half *A, __half *B, float *C,
// int m, int n)
// {
// 	if(nn <= 128)
// 	{
...
// 	startTimer();
// 	cublasStatus_t t = cublasGemmEx(ctxt.cublas_handle, CUBLAS_OP_N,
// CUBLAS_OP_T, mmm, nn/2, nn/2,
...
// 	{
// 		startTimer();
// 		cublasGemmEx(ctxt.cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T, i, i,
// k,
...
//     	);
// 		float tmp = stopTimer();
// 		printf("GEMM size %dx%dx%d takes %.3f ms, exec rate %.3f
// TFLOPS\n", i, i, k,  tmp,
//                 2.0*i*i*k/(tmp*1e9));
