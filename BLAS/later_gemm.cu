...
size_t OC_gemm::tile_size() {
  ...
  do {
    ...
    tn = N / i;
    i *= 2;
    if (is_mul_overflow(tm, K) || is_mul_overflow(tn, K))
      continue;
    mem_per_stream =
        (size_t(tm * K) + size_t(tm * tn) + size_t(K * tn)) * sizeof(float) +
        (size_t(tm * K) + size_t(K * tn)) * sizeof(half);
    if (mem_per_stream < free_mem_stream)
      break;
  } while (true);
  return mem_per_stream;
}

OC_gemm::OC_gemm(int _M, int _N, int _K, std::shared_ptr<Mem_pool> _pool,
                 size_t _mem_limit, int _num_stream)
    : M(_M), N(_N), K(_K), pool(_pool), mem_limit(_mem_limit),
      num_stream(_num_stream), streams(_num_stream), handles(_num_stream),
      A_tiles(_num_stream), B_tiles(_num_stream), C_tiles(_num_stream),
      fA_tiles(_num_stream), fB_tiles(_num_stream) {
  ...
  for (int i = 0; i < num_stream; i++) {
    fA_tiles[i] =
        reinterpret_cast<float *>(pool->allocate(sizeof(float) * tm * K));
    fB_tiles[i] =
        reinterpret_cast<float *>(pool->allocate(sizeof(float) * tn * K));
    C_tiles[i] =
        reinterpret_cast<float *>(pool->allocate(sizeof(float) * tn * tm));
    A_tiles[i] =
        reinterpret_cast<half *>(pool->allocate(sizeof(half) * tm * K));
    B_tiles[i] =
        reinterpret_cast<half *>(pool->allocate(sizeof(half) * tm * K));
  }
}
...
void OC_gemm::gemm(cublasOperation_t transa, cublasOperation_t transb,
  ...
  for (size_t i = 0; i < (M / tm); i++) {
    ...
    if (transa == CUBLAS_OP_N) {
      cublasChk(cublasSetMatrixAsync(tm, K, sizeof(half), &A[i * tm], M,
    ...
    for (size_t j = 0; j < (N / tn); j++) {
      ...
      cublasChk(cublasGemmEx(handles[stream_id], transa, transb, tm, tn, K,
...
__global__ void __kernel_AB2half(const float *__restrict__ fA,
                                 const float *__restrict__ fB,
                                 half *__restrict__ hA, half *__restrict__ hB,
...
void OC_gemm::gemm(cublasOperation_t transa, cublasOperation_t transb,
  ...
  for (size_t i = 0; i < (M / tm); i++) {
    ...
    auto stream = streams[stream_id];
    if (transa == CUBLAS_OP_N) {
      cublasChk(cublasSetMatrixAsync(tm, K, sizeof(float), &A[i * tm], M,
                                     A_tiles[stream_id], tlda, stream));
    ...
    for (size_t j = 0; j < (N / tn); j++) {
      ...
      auto size_A = tm * K;
      auto size_B = tn * K;
      __kernel_AB2half<<<(std::max(size_A, size_B) / 1024) + 1, 1024, 0,
                         stream>>>(fA_tiles[stream_id], fB_tiles[stream_id],
                                   A_tiles[stream_id], B_tiles[stream_id],
                                   size_A, size_B);
      cublasChk(cublasGemmEx(handles[stream_id], transa, transb, tm, tn, K,
                             &alpha, A_tiles[stream_id], CUDA_R_16F, tlda,
                             B_tiles[stream_id], CUDA_R_16F, tldb, &beta,
...
#pragma omp parallel for
    ...
    /*
     * Usage:
     *  allocate memory: float* p = reinterpret_cast<float
*>(pool->allocate(size));
