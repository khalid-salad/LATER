...
void later_rsyrk(cublasHandle_t handle, int n, int k, float alpha, float* A, int lda, float beta, float* C, int ldc,
    ...
    if (n <= BLOCKSIZE) {
        ...
        cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_T, n, n, k, &alpha, ah, CUDA_R_16F, n, ah, CUDA_R_16F, n, &beta, C,
        ...
        cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_T, n / 2, n / 2, k, &alpha, Ah, CUDA_R_16F, n / 2, Bh, CUDA_R_16F,
                     n / 2, &beta, C + n / 2, CUDA_R_32F, ldc, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);

        later_rsyrk(handle, n / 2, k, alpha, A + n / 2, lda, beta, C + n / 2 + ldc / 2 * n, ldc, hwork);
    }
}