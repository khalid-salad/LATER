...
int main(int argc, char *argv[]) {
...

  {
    ...
    int lwork;
    cusolverDnSsytrd_bufferSize(cusolver_handle, CUBLAS_FILL_MODE_LOWER, n, A,
                                lda, D, E, tau, &lwork);
    ...
    startTimer();
    cusolverDnSsytrd(cusolver_handle, CUBLAS_FILL_MODE_LOWER, n, A, lda, D, E,
  ...
  }
  {
    ...
    cusolverDnSsyevd_bufferSize(cusolver_handle, jobz, CUBLAS_FILL_MODE_LOWER,
                                n, A, lda, D, &lwork);
    ...
    cudaMalloc(&devInfo, sizeof(*devInfo));
    startTimer();
    cusolverDnSsyevd(cusolver_handle, jobz, CUBLAS_FILL_MODE_LOWER, n, A, lda,
                     D, work, lwork, devInfo);
    float ms = stopTimer(); // in milliseconds
    int info;
    cudaMemcpy(&info, devInfo, sizeof(info), cudaMemcpyDeviceToHost);
    float GFLOPS = 4.0 / 3.0 * n * n * n / (ms * 1.0e6);
    printf(
        " Ssyevd (novector) info=%d took time %.0f milliseconds GFLOPS: %.0f\n",
        info, ms, GFLOPS);
  }
  {
    curandSetPseudoRandomGeneratorSeed(gen, 1234ULL);
    ...
    cudaMemcpy(&info, devInfo, sizeof(info), cudaMemcpyDeviceToHost);
    float GFLOPS = 4.0 / 3.0 * n * n * n / (ms * 1.0e6);
    printf(" Ssyevd (with vector) info=%d took time %.0f milliseconds GFLOPS: "
           "%.0f\n",
           info, ms, GFLOPS);
  }
#ifdef MAGMA
