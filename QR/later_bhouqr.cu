...
/*
This routine performs block Householder QR factorization. It uses rhouqr as
panel factorization. The input A stores the original matrix A that will be
factorized The output A stores the Householder vectors Y The output W stores the
W matrix of WY representation THe output R stores the upper triangular matrix
*/
...
__global__ void copyAndClear(int m, int n, float *da, int lda, float *db,
                             int ldb) {
...
void formW(cudaCtxt ctxt, int m, int n, float *W, int ldw, float *Y, int ldy,
           float *work) {
  ...
  cublasSgemm(ctxt.cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, n / 2, n / 2, m,
              &sone, Y, ldy, W + ldw / 2 * n, ldw, &szero, work, n / 2);
...
void later_bhouqr(int m, int n, float *A, int lda, float *W, int ldw, float *R,
  ...
  for (int i = 0; i < n; i += NMIN) {
    ...
    // printMatrixDeviceBlock_("oA2.csv", m - i,n - i - nb, A+(i+nb)*lda+i,
    // lda);

    // panel factorization
    startTimer();
    later_rhouqr(ctxt, m - i, nb, A + i * lda + i, lda, W + i * lda + i, ldw,
                 R + i * ldr + i, ldr, work, lwork, hwork, lhwork, U);
    ...
    startTimer();
    if (n - i > NMIN) {
      cublasSgemm(ctxt.cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, nb, n - i - nb,
                  m - i, &sone, W + i * lda + i, ldw, A + (i + nb) * lda + i,
                  lda, &szero, work, nb);

      cublasSgemm(ctxt.cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, m - i,
                  n - i - nb, nb, &snegone, A + i * lda + i, lda, work, nb,
                  &sone, A + (i + nb) * lda + i, lda);
      // printMatrixDeviceBlock_("A2.csv", m - i,n - i - nb, A+(i+nb)*lda+i,
      // lda); break;

      dim3 grid((nb + 1) / 32, (n - i - nb + 1) / 32);
      dim3 block(32, 32);
      copyAndClear<<<grid, block>>>(nb, n - i - nb, A + (i + nb) * lda + i, lda,
                                    R + (i + nb) * ldr + i, ldr);
    }
    gemmTime += stopTimer();
    ...
    // update W
    if (i != 0 && wflag) {
      cublasSgemm(ctxt.cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, i, nb, m, &sone,
                  A, lda, W + i * lda, ldw, &szero, work, i);

      cublasSgemm(ctxt.cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, m, nb, i,
                  &snegone, W, ldw, work, i, &sone, W + i * lda, ldw);
    }
