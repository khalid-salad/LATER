...
void qr(cudaCtxt ctxt, int m, int n, float *A, int lda, float *W, int ldw,
        float *R, int ldr, float *work, int lwork, __half *hwork, int lhwork,
        float *U);
...
__global__ void setZero(int m, int n, float *I, int ldi) {
...
void qr(cudaCtxt ctxt, int m, int n, float *A, int lda, float *W, int ldw,
  ...
  if (n <= NMIN) {
    ...
    cublasStrsm(ctxt.cublas_handle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_LOWER,
                CUBLAS_OP_T, CUBLAS_DIAG_UNIT, m, n, &sone, A, lda, W, ldw);
    return;
  }
  ...
  if (n / 2 <= 128 || m <= 128) {
    cublasSgemm(ctxt.cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, n / 2, n / 2, m,
                &sone, W, ldw, A + lda / 2 * n, lda, &szero, work, n / 2);
    cublasSgemm(ctxt.cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n / 2, n / 2,
                &snegone, A, lda, work, n / 2, &sone, A + lda / 2 * n, lda);
  } else {
    __half *Ah = hwork;
    ...
    auto status = cublasGemmEx(ctxt.cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N,
                               n / 2, n / 2, m, &sone, Ah, CUDA_R_16F, m, Bh,
                               CUDA_R_16F, m, &szero, work, CUDA_R_32F, n / 2,
    ...
    CHECK_KERNEL();
    status = cublasGemmEx(ctxt.cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, m,
                          n / 2, n / 2, &snegone, Ah, CUDA_R_16F, m, Bh,
                          CUDA_R_16F, n / 2, &sone, A + lda / 2 * n, CUDA_R_32F,
                          lda, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
    assert(status == CUBLAS_STATUS_SUCCESS);
    CHECK_KERNEL();
  }

  qr(ctxt, m - n / 2, n / 2, A + lda / 2 * n + n / 2, lda,
     W + ldw / 2 * n + n / 2, ldw, R + n / 2 * ldr + n / 2, ldr, work, lwork,
     hwork, lhwork, U);

  dim3 gridDim1((n / 2 + 31) / 32, (n / 2 + 31) / 32);
  dim3 blockDim1(32, 32);

  deviceCopy<<<gridDim1, blockDim1>>>(n / 2, n / 2, A + lda / 2 * n, lda,
                                      R + ldr / 2 * n, ldr);
  ...
  if (n / 2 <= 128 || m <= 128) {
    cublasSgemm(ctxt.cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, n / 2, n / 2, m,
                &sone, A, lda, W + ldw / 2 * n, ldw, &szero, work, n / 2);
    cublasSgemm(ctxt.cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n / 2, n / 2,
                &snegone, W, ldw, work, n / 2, &sone, W + ldw / 2 * n, ldw);
  } else {
    __half *Ah = hwork;
    __half *Bh = hwork + m / 2 * n;
    // startTimer();
    ...
    cublasGemmEx(ctxt.cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, n / 2, n / 2, m,
                 &sone, Ah, CUDA_R_16F, m, Bh, CUDA_R_16F, m, &szero, work,
                 CUDA_R_32F, n / 2, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
    CHECK_KERNEL();
    s2h<<<gridDimA, blockDimA>>>(m, n / 2, W, ldw, Ah, m);
    ...
    cublasGemmEx(ctxt.cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n / 2, n / 2,
                 &snegone, Ah, CUDA_R_16F, m, Bh, CUDA_R_16F, n / 2, &sone,
                 W + ldw / 2 * n, CUDA_R_32F, ldw, CUDA_R_32F,
...
__global__ void getU(int m, int n, float *a, int lda, float *u, int ldr) {
  int i = threadIdx.x + blockDim.x * blockIdx.x;
  int j = threadIdx.y + blockDim.y * blockIdx.y;
  if (i < m && j < n) {
    if (i > j)
...
void reconstructY(cudaCtxt ctxt, int m, int n, float *dA, float *dU, int lda) {
  ...
  cusolverDnSgetrf_bufferSize(ctxt.cusolver_handle, n, n, dA, lda, &lwork);

  cudaMalloc((void **)&d_info, sizeof(int));
  cudaMalloc((void **)&d_work, sizeof(float) * lwork);

  cusolverDnSgetrf(ctxt.cusolver_handle, n, n, dA, lda, d_work, NULL, d_info);
  ...
  float sone = 1.0;
  cublasStrsm(ctxt.cublas_handle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_UPPER,
              CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT, m - n, n, &sone, dU, n, dA + n,
              lda);
}
