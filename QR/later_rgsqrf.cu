...
void qr(cudaCtxt ctxt, int m, int n, float *A, int lda, float *R, int ldr, float *work, int lwork, __half *hwork,
        int lhwork) {
    ...
    cublasGemmEx(ctxt.cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, n / 2, n / 2, m, &sone, Ah, CUDA_R_16F, lda, Bh,
                 CUDA_R_16F, lda, &szero, &R[n / 2 * ldr], CUDA_R_32F, ldr, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
    dim3 gridDim2((n + 31) / 32, (n + 31) / 31);
    s2h<<<gridDim2, blockDim>>>(n / 2, n / 2, &R[n / 2 * ldr], ldr, Bh, n / 2);
    cublasGemmEx(ctxt.cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n / 2, n / 2, &snegone, Ah, CUDA_R_16F, m, Bh,
                 CUDA_R_16F, n / 2, &sone, &A[n / 2 * lda], CUDA_R_32F, lda, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
