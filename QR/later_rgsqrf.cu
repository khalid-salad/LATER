...
void qr(cudaCtxt ctxt, int m, int n, float *A, int lda, float *R, int ldr,
  ...
  cublasGemmEx(ctxt.cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, n / 2, n / 2, m,
               &sone, Ah, CUDA_R_16F, lda, Bh, CUDA_R_16F, lda, &szero,
               &R[n / 2 * ldr], CUDA_R_32F, ldr, CUDA_R_32F,
  ...
  cublasGemmEx(ctxt.cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n / 2, n / 2,
               ...
               &A[n / 2 * lda], CUDA_R_32F, lda, CUDA_R_32F,
               CUBLAS_GEMM_DEFAULT_TENSOR_OP);
  qr(ctxt, m, n / 2, &A[n / 2 * lda], lda, &R[n / 2 + n / 2 * ldr], ldr, work,
     lwork, hwork, lhwork);
