...
void later_ormqr(int m, int n, float *W, int ldw, float *Y, int ldy,
  ...
  cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, n / 2, n / 2, m, &sone, Y, ldy,
              W + ldw / 2 * n, ldw, &szero, work, n / 2);

  cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n / 2, n / 2, &snegone, W,
              ldw, work, n / 2, &sone, W + ldw / 2 * n, ldw);
...
void later_ormqr2(int m, int n, float *W, int ldw, float *Y, int ldy,
  ...
  cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_T, m, n, n, &snegone, W,
               CUDA_R_32F, ldw, Y, CUDA_R_32F, ldy, &sone, work, CUDA_R_32F, m,
               CUDA_R_32F, CUBLAS_GEMM_DEFAULT);
